...
index <- createDataPartition(strokeup$had_stroke,p=0.7,list=FALSE)
trainup <- strokeup[index,]
testup <- strokeup[-index,]
...
#Now need to upsample the normalized/dummy data

train_norm <- stroke_norm[index, ]
test_norm <- stroke_norm[-index, ]

trainup_norm <- upSample(x = train_norm[,-ncol(trainup)],
                  y=as.factor(train_norm$had_stroke))
trainup_norm$had_stroke <- trainup_norm$Class
trainup_norm$Class <- NULL
...
#Checking the structure of trainup_norm to see if we need to convert to numeric
str(trainup_norm$had_stroke)

#Converting to numeric
trainup_norm$had_stroke <- as.numeric(trainup_norm$had_stroke)-1
str(trainup_norm$had_stroke)
table(trainup_norm$had_stroke)

#My ANN model 
stroke.ann <- neuralnet(formula = had_stroke ~ .,
                              data = trainup_norm, hidden = 1, stepmax = 1e+06)

# Visualize the network topology
plot(stroke.ann)

# Create a confusion matrix to see model accuracy
stroke_ann_pred <- predict(stroke.ann,test_norm,type = "response")
stroke_ann_pred <- ifelse(stroke_ann_pred > 0.8, 1, 0)
confusionMatrix(as.factor(stroke_ann_pred),as.factor(test_norm$had_stroke),positive = "1")
...
stroke_classifier = ksvm(had_stroke ~., data = trainup, kernel = "vanilladot")
stroke_classifier

stroke_predictions = predict(stroke_classifier, test)

table(stroke_predictions, test$had_stroke)
...
library(C50)
stroke_decision_tree <- C5.0(trainup[-11], as.factor(trainup$had_stroke))
...
stroke_dt_pred <- predict(stroke_decision_tree, test)
#Creating a confusion matrix
library(caret)
confusionMatrix(as.factor(stroke_dt_pred),as.factor(test$had_stroke),positive = "1")
